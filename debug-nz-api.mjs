...
async function main() {
  ...
  for (const day of diary.dates) {
    for (const call of (day.calls || [])) {
      for (const subj of (call.subjects || [])) {
        ...
        if (subj.lesson) {
          const lessons = Array.isArray(subj.lesson) ? subj.lesson : [subj.lesson];
  ...
  const lastDayWithCalls = diary.dates.filter(d => d.calls && d.calls.length > 0).slice(-1)[0];
  if (lastDayWithCalls) {
  ...
  try {
    ...
    if (perfData.subjects) {
      ...
      console.log('Full response:', JSON.stringify(perfData, null, 2).slice(0, 2000));
    }
  } catch (e) {
  ...
  try {
    ...
    if (perfData2.subjects && perfData2.subjects.length > 0) {
      ...
      console.log(`Fetching grades for subject: ${firstSubj.subject_name || firstSubj.name} (id: ${subjId})`);

      const sgRes = await fetch(`${API_BASE}/schedule/subject-grades`, {
        method: 'POST',
        headers: authHeaders,
        body: JSON.stringify({ start_date: start, end_date: end, subject_id: subjId }),
      });
