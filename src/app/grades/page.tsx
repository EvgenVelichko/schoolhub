...
export default function GradesPage() {
  ...
  const subjectStats = React.useMemo(() => {
    ...
    const stats: Record<
      string,
      { total: number; count: number; name: string; history: number[] }
    > = {};
    ...
    allGrades.forEach((g: any) => {
      ...
      if (!stats[g.subject])
        stats[g.subject] = { total: 0, count: 0, name: g.subject, history: [] };
    ...
    return Object.values(stats)
      .map((s) => {
        ...
        const recentAvg =
          s.history.slice(0, 3).reduce((a, b) => a + b, 0) /
          Math.min(s.history.length, 3);
  ...
  if (loading)
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-background gap-4">
        ...
        <p className="text-xs font-black uppercase tracking-[0.3em] text-primary/50">
          Завантажуємо журнал...
        </p>
      </div>
    );
    ...
    <div className="p-3 sm:p-4 md:p-8 max-w-7xl mx-auto space-y-6 sm:space-y-10 animate-reveal pb-32 overflow-x-hidden">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
        <Card className="glass-panel border-0 rounded-[1.5rem] sm:rounded-[2rem] md:rounded-[2.5rem] p-4 sm:p-6 md:p-8 col-span-1 flex flex-col justify-between group transition-all">
          ...
          <div>
            <p className="text-[8px] sm:text-[9px] font-black uppercase tracking-widest text-muted-foreground mb-1">
              Середній бал
            </p>
            <h2 className="text-3xl sm:text-5xl md:text-6xl font-black italic text-white tracking-tighter">
              {overallAverage}
            </h2>
          </div>
        </Card>
          ...
          <div className="relative z-10 h-full flex flex-col justify-between">
            <div className="space-y-2">
              <h3 className="text-xl md:text-2xl font-bold flex items-center gap-3">
                <Calculator className="size-6 text-primary" /> Прогноз Успіху
              </h3>
              <p className="text-muted-foreground text-xs md:text-sm max-w-lg opacity-80">
                Ваша поточна активність показує позитивну динаміку. Отримайте ще
                кілька оцінок "12", щоб підвищити рейтинг.
              </p>
            </div>

            <div className="mt-6 md:mt-8 flex items-end justify-between">
              <div className="flex items-end gap-4 md:gap-6">
                <div className="text-5xl md:text-7xl font-black text-primary italic leading-none">
                  ~{(parseFloat(String(overallAverage)) + 0.4).toFixed(1)}
                </div>
      ...
      <Tabs defaultValue="list" className="space-y-6 sm:space-y-8">
        <TabsList className="glass-panel p-1 rounded-xl sm:rounded-2xl h-11 sm:h-14 border-0 w-full md:w-fit flex overflow-x-auto scrollbar-none">
          <TabsTrigger
            value="list"
            className="rounded-lg sm:rounded-xl px-4 sm:px-6 md:px-10 h-full font-bold text-xs sm:text-sm data-[state=active]:cyber-gradient shrink-0"
          >
            Всі оцінки
          </TabsTrigger>
          <TabsTrigger
                    ...
                    <Card className="glass-panel border-0 rounded-[1.5rem] md:rounded-[2rem] p-5 flex items-center justify-between group transition-all hover:border-primary/40">
                      <div className="flex items-center gap-4 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          <h4 className="font-bold text-white text-base md:text-lg truncate leading-tight">
                            {grade.subject}
                          </h4>
                          <div className="flex flex-col mt-0.5">
                            ...
                            <span className="text-[9px] font-bold text-primary/80 uppercase mt-0.5">
                              {grade.date &&
                              !isNaN(parseISO(grade.date).getTime())
                                ? format(parseISO(grade.date), "d MMM", {
                                    locale: uk,
                                  })
                                : "—"}
                            </span>
        ...
        <TabsContent
          value="subjects"
          className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 animate-in fade-in slide-in-from-bottom-4"
        >
          {subjectStats.map((stat, i) => (
            <Card
              key={i}
              className="glass-panel border-0 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 transition-all group relative overflow-hidden"
            >
              <div className="flex justify-between items-start mb-8 relative z-10">
                <div className="space-y-1 min-w-0">
                  <h4 className="font-bold text-white text-lg md:text-xl truncate w-full group-hover:text-primary transition-colors">
                    {stat.name}
                  </h4>
                  <p className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">
                    Проаналізовано {stat.count} оцінок
                  </p>
                </div>
                <div
                  className={cn(
              ...
              <div className="space-y-4 relative z-10">
                ...
                <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden border border-white/5">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{
                      width: `${(parseFloat(stat.average) / 12) * 100}%`,
                    }}
                    transition={{ duration: 1, ease: "easeOut" }}
                    className={cn(
                      "h-full",
                      stat.trend === "up" ? "bg-green-500" : "cyber-gradient",
                    )}
                  />
                </div>
              </div>
        ...
        >
          {(() => {
            const selectedStat = subjectStats.find(
              (s) => s.name === calcSubject,
            );
            const target = parseFloat(calcTarget);

            function calcNeeded(
              stat: typeof selectedStat,
              targetAvg: number,
              gradeToGet: number,
            ) {
              if (!stat || isNaN(targetAvg) || targetAvg <= 0 || targetAvg > 12)
                return null;
              if (parseFloat(stat.average) >= targetAvg) return 0;
              if (gradeToGet <= targetAvg) return null; // impossible
              const n = Math.ceil(
                (targetAvg * stat.count - stat.total) /
                  (gradeToGet - targetAvg),
              );
              return Math.max(n, 0);
            }
                ...
                <Card className="glass-panel border-0 rounded-[2rem] p-6 md:p-8 space-y-6">
                  <div className="space-y-1">
                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                      <Calculator className="size-5 text-primary" /> Калькулятор
                      оцінок
                    </h3>
                    <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">
                      Скільки оцінок потрібно для бажаного середнього?
                    </p>
                  </div>
                  ...
                  <div className="space-y-4">
                        ...
                        <SelectContent className="glass-panel border-white/10 rounded-xl">
                          {subjectStats.map((s) => (
                            <SelectItem
                              key={s.name}
                              value={s.name}
                              className="rounded-lg focus:bg-primary/20 font-bold text-xs"
                            >
                              {s.name} — {s.average}
                            </SelectItem>
                          ))}
                    ...
                    {selectedStat && (
                      <motion.div
                        ...
                        className="p-4 rounded-2xl bg-primary/5 border border-primary/10 space-y-1"
                      >
                        <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">
                          Поточний середній
                        </p>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {selectedStat.count} оцінок, сума {selectedStat.total}
                        </p>
                      </motion.div>
                    )}
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  target <= 12 ? (
                      ...
                      >
                        <Card className="glass-panel border-0 rounded-[2rem] p-6 md:p-8 text-center space-y-4">
                          ...
                          <p className="text-muted-foreground text-sm">
                            Ваш поточний середній{" "}
                            <span className="text-white font-bold">
                              {selectedStat.average}
                            </span>{" "}
                            вже {"\u2265"} {target.toFixed(1)}
                          </p>
                        </Card>
                      </motion.div>
                    ) : (
                            ...
                            <Card className="glass-panel border-0 rounded-[2rem] p-5 md:p-6 flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  ...
                                  <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">
                                    Для середнього {target.toFixed(1)}
                                  </p>
                                </div>
                              </div>
                              <div className="text-right">
                    ...
                    <Card className="glass-panel border-0 rounded-[2rem] p-8 md:p-12 text-center space-y-4">
                      ...
                      <p className="text-muted-foreground text-lg font-medium italic">
                        Оберіть предмет та бажаний середній бал
                      </p>
                    </Card>
                  )}
                </div>
