...
export default function GradesPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-10 animate-reveal pb-32 overflow-x-hidden">
      <header className="space-y-3">
        <h1 className="text-3xl md:text-6xl font-headline font-bold text-white text-glow tracking-tighter">
          Журнал <span className="text-primary">Досягнень</span>
        </h1>
        <p className="text-muted-foreground text-base md:text-xl max-w-2xl leading-relaxed opacity-70">
          Ваша академічна історія. Аналізуйте тренди та покращуйте результати.
        </p>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
        <Card className="glass-panel border-0 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 md:col-span-1 flex flex-col justify-between group transition-all shadow-2xl">
          <div className="size-12 md:size-14 rounded-2xl bg-primary/10 flex items-center justify-center border border-primary/20 mb-6 md:mb-8">
             <TrendingUp className="text-primary size-6 md:size-7" />
          </div>
          <div>
            <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground mb-1">Середній бал</p>
            <h2 className="text-5xl md:text-6xl font-black italic text-white tracking-tighter">{overallAverage}</h2>
          </div>
        </Card>

        <Card className="glass-panel border-0 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 md:col-span-3 overflow-hidden relative shadow-2xl">
      ...
      <Tabs defaultValue="list" className="space-y-8">
        <TabsList className="glass-panel p-1 rounded-2xl h-14 border-0 w-full md:w-fit flex shadow-xl overflow-x-auto scrollbar-none">
          <TabsTrigger value="list" className="rounded-xl px-6 md:px-10 h-full font-bold data-[state=active]:cyber-gradient shrink-0">Всі оцінки</TabsTrigger>
          <TabsTrigger value="subjects" className="rounded-xl px-6 md:px-10 h-full font-bold data-[state=active]:cyber-gradient shrink-0">Аналітика</TabsTrigger>
        </TabsList>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex flex-col mt-0.5">
                          ...
                          <span className="text-[9px] font-bold text-primary/80 uppercase mt-0.5">
                            {format(parseISO(grade.date), 'd MMM', { locale: uk })}
                          </span>
                        </div>
                      </div>
                    </div>
