...
const ROLES = ["user", "tester", "admin", "owner"]

export default function AdminHub() {
  const { user: currentUser } = useUser()
  const db = useFirestore()
  
  const userProfileRef = React.useMemo(() => (currentUser ? doc(db, "users", currentUser.uid) : null), [db, currentUser])
  const { data: profile, loading: profileLoading } = useDoc(userProfileRef)

  const usersQuery = React.useMemo(() => query(collection(db, "users"), orderBy("displayName", "asc")), [db])
  const { data: users } = useCollection(usersQuery)

  const reportsQuery = React.useMemo(() => query(collection(db, "reports"), orderBy("status", "asc"), orderBy("createdAt", "desc")), [db])
  ...
  const reportMessagesQuery = React.useMemo(() => (activeReportId ? query(
    collection(db, "reports", activeReportId, "messages"),
    orderBy("createdAt", "desc")
  ) : null), [db, activeReportId])
  const { data: supportMessages } = useCollection(reportMessagesQuery)

  const handleUpdateUser = async (userId: string, data: any, targetRole: string) => {
    // Власник недоторканний
    if (targetRole === 'owner' && (data.isBanned !== undefined || data.isMuted !== undefined)) {
      toast({ title: "Помилка", description: "Власник недоторканний!", variant: "destructive" })
      return
    }
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-10 animate-reveal pb-20 scrollbar-none">
      <header className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl md:text-5xl font-headline font-bold text-white">Адмін <span className="text-primary">Hub</span></h1>
            <p className="text-muted-foreground text-sm uppercase tracking-[0.3em] font-black mt-1">Керування системою</p>
          </div>
        </div>
        <Badge variant="outline" className="border-primary/20 bg-primary/5 px-6 py-2 text-primary font-black uppercase tracking-widest rounded-xl">{role}</Badge>
      </header>

      <Tabs defaultValue="users" className="space-y-8">
        <TabsList className="glass-panel p-1.5 rounded-2xl h-16 border-0 w-full md:w-auto">
          <TabsTrigger value="users" className="rounded-xl px-10 h-full flex-1 md:flex-none font-bold">Користувачі</TabsTrigger>
          <TabsTrigger value="support" className="rounded-xl px-10 h-full flex-1 md:flex-none font-bold">Підтримка</TabsTrigger>
        </TabsList>
              ...
              <Table>
                <TableHeader className="bg-white/5">
                  <TableRow className="border-white/5 hover:bg-transparent">
                    <TableHead className="px-8 font-black uppercase text-[10px] tracking-widest text-muted-foreground">Користувач</TableHead>
                    <TableHead className="font-black uppercase text-[10px] tracking-widest text-muted-foreground">Роль</TableHead>
                    <TableHead className="font-black uppercase text-[10px] tracking-widest text-muted-foreground">Баланс</TableHead>
                    <TableHead className="font-black uppercase text-[10px] tracking-widest text-muted-foreground">Статус</TableHead>
                    <TableHead className="text-right px-8 font-black uppercase text-[10px] tracking-widest text-muted-foreground">Дії</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {users?.map((u: any) => {
                      ...
                      <TableRow key={u.id} className="hover:bg-white/5 transition-colors border-white/5">
                        <TableCell className="px-8 flex items-center gap-4 py-6">
                          ...
                          <div className="flex flex-col">
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-white text-lg leading-none">{u.displayName}</span>
                              {isTargetOwner && <Crown className="size-4 text-yellow-500" />}
                            </div>
                            <span className="text-[10px] text-primary font-black uppercase tracking-widest mt-2">{(u.gradeLevel || "Новачок").toString().trim()}</span>
                          </div>
                        </TableCell>
                        <TableCell>
                          <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                              <Button variant="outline" size="sm" className="bg-white/5 border-white/10 rounded-xl h-9 hover:bg-primary/10 transition-all gap-2 group">
                            ...
                            <DropdownMenuContent className="glass-panel border-white/10 rounded-xl p-2 w-48">
                              {ROLES.map((r) => (
                                <DropdownMenuItem 
                                  key={r} 
                                  className={cn("rounded-lg font-bold uppercase text-[10px] tracking-widest mb-1 text-white", u.role === r ? "bg-primary/20 text-primary" : "hover:bg-white/5")}
                                  onClick={() => handleUpdateUser(u.id, { role: r }, u.role)}
                                >
                                  {r}
                                </DropdownMenuItem>
                              ))}
                        ...
                        <TableCell>
                          <div className="flex items-center gap-3">
                            <div className="flex items-center gap-2 bg-yellow-500/10 text-yellow-500 px-3 py-1.5 rounded-lg border border-yellow-500/20">
                              ...
                              <span className="font-black text-xs">{isTargetOwner ? "∞" : (u.coins || 0)}</span>
                            </div>
                            {!isTargetOwner && (
                              <Button variant="ghost" size="icon" className="size-8 rounded-lg hover:bg-green-500/10 hover:text-green-500" onClick={() => handleAddCoins(u.id, 100)}>
                        ...
                        <TableCell>
                          <div className="flex gap-2">
                            {u.isBanned && <Badge variant="destructive" className="rounded-md font-black text-[9px] tracking-widest px-2 py-1">BANNED</Badge>}
                            {u.isMuted && <Badge className="bg-orange-500/10 text-orange-500 border-orange-500/20 rounded-md font-black text-[9px] tracking-widest px-2 py-1">MUTED</Badge>}
                            {!u.isBanned && !u.isMuted && <Badge className="bg-green-500/10 text-green-500 border-green-500/20 rounded-md font-black text-[9px] tracking-widest px-2 py-1">ACTIVE</Badge>}
                          </div>
                        </TableCell>
                        <TableCell className="text-right px-8">
                          <div className="flex justify-end gap-2">
                            <Button 
                              ...
                              className={cn("rounded-xl", u.isMuted ? "text-orange-500 bg-orange-500/10" : "text-muted-foreground hover:bg-white/5", isTargetOwner && "opacity-20 cursor-not-allowed")} 
                              onClick={() => handleUpdateUser(u.id, { isMuted: !u.isMuted }, u.role)}
                            >
                              {u.isMuted ? <MicOff className="size-4" /> : <Mic className="size-4" />}
                            </Button>
                            <Button 
                              variant="ghost" 
                              size="icon" 
                              disabled={isTargetOwner}
                              className={cn("rounded-xl", u.isBanned ? "text-green-500 bg-green-500/10" : "text-destructive hover:bg-destructive/10", isTargetOwner && "opacity-20 cursor-not-allowed")} 
                              onClick={() => handleUpdateUser(u.id, { isBanned: !u.isBanned }, u.role)}
                            >
        ...
        <TabsContent value="support" className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <Card className="glass-panel border-0 p-8 space-y-6 rounded-[2.5rem] h-fit overflow-y-auto max-h-[700px] scrollbar-none">
            ...
            <div className="space-y-3">
                ...
                <div key={t.id} className="group relative">
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      {t.status === 'closed' && <Badge className="bg-white/20 text-[8px] rounded h-4">CLOSED</Badge>}
                    </div>
                    <p className="text-[11px] text-white/50 truncate mt-2 font-medium">{t.lastMessage}</p>
                  </button>
                  <div className="absolute right-3 top-4 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button 
                      ...
                      onClick={(e) => { e.stopPropagation(); handleCloseReport(t.id); }}
                    >
                    ...
                    <Button 
                      ...
                      onClick={(e) => { e.stopPropagation(); handleDeleteReport(t.id); }}
                    >
              ...
              {!reports?.length && <p className="text-center py-10 text-muted-foreground opacity-30 italic">Немає активних звернень</p>}
            </div>
          </Card>
            ...
            {activeReportId ? (
              <>
                <div className="p-6 border-b border-white/5 bg-white/5 flex items-center justify-between">
                   ...
                   <Button variant="ghost" size="icon" onClick={() => setActiveReportId(null)} className="rounded-xl"><XCircle className="size-5" /></Button>
                </div>
                ...
                <div className="p-8 border-t border-white/5 space-y-4 bg-black/30 backdrop-blur-md">
                  <div className="flex flex-wrap gap-2">
                    {QUICK_REPLIES.map((q, idx) => (
                      <Button key={idx} variant="outline" size="sm" className="text-[10px] font-bold rounded-xl bg-white/5 h-8 px-4 text-white hover:bg-primary/20 hover:border-primary/50" onClick={() => handleSendReply(q)}>
                        {q}
                      </Button>
                    ))}
                  </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground/30 space-y-4">
                ...
                <p className="text-xl font-bold uppercase tracking-widest">Виберіть репорт</p>
              </div>
            )}
          </Card>
        </TabsContent>
