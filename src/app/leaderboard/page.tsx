...
export default function Leaderboard() {
  ...
  const currentUserRef = React.useMemo(
  ...
  const getDisplayName = (userData: any) => {
    if (isGhostUser(userData) && userData.id !== user?.uid)
          ...
          return (
            <Card
              ...
              className={cn(
                ...
                !isGhost &&
                  "cursor-pointer hover:scale-[1.03] sm:hover:scale-[1.07]",
            ...
            >
              ...
              >
                {isGhost ? (
                  <Ghost className="size-5 sm:size-8 text-slate-400" />
                ) : index === 0 ? (
                  <Crown className="size-5 sm:size-8" />
                ) : (
                  <Medal className="size-5 sm:size-8" />
                )}
              </div>
              <CardHeader className="text-center pb-1 sm:pb-2 px-2 sm:px-6">
                <div className="relative inline-block mx-auto mb-2 sm:mb-4">
                  ...
                  >
                    <AvatarImage
                      src={getDisplayAvatar(userData) || undefined}
                    />
                    <AvatarFallback
                      className={cn(isGhost && "bg-slate-800 text-slate-500")}
                    >
                ...
                <p className="text-[9px] sm:text-sm text-muted-foreground truncate">
                  {isGhost
                    ? "Ghost Mode"
                    : isStealthUser(userData) && userData.id !== user?.uid
                      ? "Офлайн"
                      : userData.gradeLevel ||
                        "Рівень " + (userData.level || 1)}
                </p>
              </CardHeader>
          ...
          <div className="divide-y divide-white/5">
            {loading ? (
              ...
              topUsers?.slice(3).map((userData: any, index: number) => {
                const isGhost =
                  isGhostUser(userData) && userData.id !== user?.uid;
                return (
                  <div
                    key={userData.id}
                    className={cn(
                  ...
                  >
                    <div className="flex items-center gap-2 sm:gap-4 min-w-0">
                      ...
                      >
                        ...
                        <AvatarFallback
                          className={cn(
                            "text-xs",
                            isGhost && "bg-slate-800 text-slate-500",
                          )}
                        >
                          {getAvatarFallback(userData)}
                        </AvatarFallback>
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[9px] sm:text-xs text-muted-foreground truncate">
                          {isGhost
                            ? "Ghost Mode"
                            : isStealthUser(userData) &&
                                userData.id !== user?.uid
                              ? "Офлайн"
                              : userData.gradeLevel || "Учень"}
                        </p>
                      </div>
                      {isGhost && (
                        <Ghost className="size-4 text-slate-500 shrink-0 ml-1" />
                      )}
                    </div>
                    <div className="text-right shrink-0 ml-2">
                      ...
                      <p className="text-[8px] sm:text-[10px] uppercase text-muted-foreground font-bold tracking-widest">
                        Досвід
                      </p>
                    </div>
                  </div>
