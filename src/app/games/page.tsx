...
export default function PlayZone() {
  ...
  const generateMath = () => {
    ...
    setMathProblem({
      q: `${a} ${op === "*" ? "×" : op} ${b}`,
      a: res,
      options: shuffleArray([res, ...Array.from(wrongOptions)]),
    });
  };
  ...
  const handleReactionClick = () => {
    ...
    if (reactionState === "go") {
      ...
      if (time < 300) {
        ...
        toast({
          title: `${time}мс — Блискавка!`,
          description: `+${earned} монет`,
        });
        setScore((s) => s + 1);
      } else if (time < 500) {
        const earned = addCoins(10);
        toast({
          title: `${time}мс — Швидко!`,
          description: `+${earned} монет`,
        });
        setScore((s) => s + 1);
      } else {
  ...
  React.useEffect(() => {
    if (colorTimeLeft === 0 && activeGame === "colorMatch") {
      ...
      toast({
        title: "Час вийшов!",
        description: `Рахунок: ${score} • +${earned} монет`,
      });
    }
  }, [colorTimeLeft]);
  ...
  const bjHit = () => {
    ...
    if (sumCards(newCards) > 21) {
      ...
      toast({
        title: "Перебір!",
        description: `${sumCards(newCards)} — програш`,
        variant: "destructive",
      });
    }
  };
      ...
      <header className="flex items-center justify-between gap-3 sm:gap-6">
        <div className="flex items-center gap-3 sm:gap-4">
          <Button
            variant="ghost"
            size="icon"
            className="size-10 sm:size-12 rounded-xl bg-white/5"
            onClick={exitGame}
          >
            <ChevronLeft className="size-5 sm:size-6 text-white" />
          </Button>
          <div>
            <h1 className="font-headline text-xl sm:text-2xl md:text-4xl font-bold text-white">
              Зона <span className="text-primary text-glow">Ігор</span>
            </h1>
            <p className="text-muted-foreground text-[10px] sm:text-xs uppercase font-black tracking-widest">
              {activeGame !== "none"
                ? `Рахунок: ${score}`
                : "Розважайся з розумом"}
            </p>
          </div>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeGame === "none" && (
          ...
          >
            <GameCard
              title="Математика"
              desc="Складні приклади на швидкість."
              icon={<Zap className="size-7 text-blue-400" />}
              onPlay={() => {
                setActiveGame("math");
                generateMath();
                setScore(0);
            ...
            <GameCard
              title="Memory 4×4"
              desc="Запам'ятай і повтори послідовність чисел."
              icon={<Binary className="size-7 text-indigo-400" />}
              onPlay={() => {
            ...
            <GameCard
              title="Клікер"
              desc="Фарми монети швидкими кліками."
              icon={<MousePointer2 className="size-7 text-primary" />}
              onPlay={() => {
            ...
            <GameCard
              title="Реакція"
              desc="Натисни як тільки екран стане зеленим."
              icon={<Timer className="size-7 text-green-400" />}
              onPlay={() => {
                setActiveGame("reaction");
                setScore(0);
                setReactionBest(9999);
                startReaction();
              }}
              tags={["Speed", "+20"]}
            />
            ...
            <GameCard
              title="Колір ≠ Слово"
              desc="Назви колір тексту, ігноруючи написане слово. 30 сек."
              icon={<Palette className="size-7 text-rose-400" />}
              onPlay={() => {
            ...
            <GameCard
              title="Анаграма"
              desc="Розшифруй перемішане слово зі шкільної тематики."
              icon={<Type className="size-7 text-amber-400" />}
              onPlay={() => {
                setActiveGame("wordScramble");
                generateWord();
                setScore(0);
              }}
              tags={["Words", "+15"]}
            ...
            <GameCard
              title="Двадцять Одне"
              desc="Набери 21, але не більше. Обіграй дилера!"
              icon={<Target className="size-7 text-emerald-400" />}
              onPlay={() => {
                setActiveGame("twentyOne");
                startBlackjack();
                setScore(0);
              }}
              tags={["Cards", "+50"]}
            ...
            <Card className="glass-panel max-w-2xl mx-auto p-8 sm:p-12 text-center space-y-8 border-0 rounded-[2.5rem]">
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 max-w-sm mx-auto">
                {mathProblem.options.map((v, i) => (
                  <Button
                    key={i}
                    className="h-16 sm:h-20 text-2xl sm:text-3xl font-black rounded-2xl bg-white/5 border border-white/10 hover:bg-primary/20 hover:border-primary/40 transition-all active:scale-95"
                    onClick={() => handleMathAnswer(v)}
                  >
                    {v}
                  </Button>
                ))}
          ...
          >
            <Card className="glass-panel max-w-2xl mx-auto p-8 sm:p-12 text-center space-y-6 border-0 rounded-[2.5rem]">
              <div className="min-h-20 flex flex-wrap items-center justify-center gap-3">
                {isShowingSequence ? (
                  memorySequence.map((n, i) => (
                    <motion.div
                      key={i}
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      transition={{ delay: i * 0.15 }}
                      className="size-14 rounded-2xl cyber-gradient flex items-center justify-center text-xl font-black text-white"
                    >
                      {n}
                    </motion.div>
                  ))
                ) : (
                  <div className="text-xl font-bold text-white/50 uppercase tracking-widest">
                    Повторіть послідовність!
                  </div>
                )}
              </div>
              <div className="text-xs text-muted-foreground">
                Введено: {userSequence.length} / {memorySequence.length}
              </div>
              <div className="grid grid-cols-4 gap-2 sm:gap-3 max-w-sm mx-auto">
                {Array.from({ length: 16 }, (_, i) => i + 1).map((n) => (
                  <Button
                    ...
                    className={cn(
                      "h-12 sm:h-14 text-lg font-black rounded-xl bg-white/5 border border-white/10 hover:bg-primary/20 transition-all active:scale-90",
                      userSequence.includes(n) &&
            ...
            <Card className="glass-panel max-w-lg mx-auto p-8 sm:p-12 text-center space-y-8 border-0 rounded-[2.5rem] relative overflow-hidden">
              {isBlocked && (
                <div className="absolute inset-0 bg-black/90 backdrop-blur-xl z-50 flex flex-col items-center justify-center p-10">
                  ...
                  <p className="text-xl font-black text-white uppercase mt-4">
                    Антибот: 5 сек
                  </p>
                </div>
              )}
              <button
                disabled={isBlocked}
                className="size-48 sm:size-60 rounded-full cyber-gradient mx-auto flex items-center justify-center active:scale-95 transition-all text-6xl sm:text-7xl font-black text-white border-8 border-white/10"
                onClick={handleClick}
              >
                12+
              </button>
              <div className="flex items-center justify-center gap-4 bg-white/5 px-6 py-4 rounded-2xl border border-white/[0.06] text-white">
                ...
                <span className="text-xs text-muted-foreground ml-2">
                  = {Math.floor(clickerCount / 30)} монет
                </span>
              </div>
            </Card>
          </motion.div>
            ...
            <Card className="glass-panel max-w-2xl mx-auto border-0 rounded-[2.5rem] overflow-hidden">
              <button
                ...
                onClick={() => {
                  ...
                  else if (reactionState === "ready" || reactionState === "go")
                    handleReactionClick();
                ...
                {reactionState === "result" && (
                  <>
                    <p className="text-6xl font-black text-white">
                      {reactionTime}
                      <span className="text-2xl text-muted-foreground">мс</span>
                    </p>
                    <p className="text-sm text-muted-foreground">
                      Рекорд: {reactionBest < 9999 ? `${reactionBest}мс` : "—"}
                    </p>
                    <p className="text-xs text-primary mt-2">
                      Натисни для нового раунду
                    </p>
                  </>
                )}
              </button>
            ...
            <Card className="glass-panel max-w-lg mx-auto p-8 sm:p-12 text-center space-y-8 border-0 rounded-[2.5rem]">
              <div className="flex items-center justify-between">
                <Badge className="bg-white/5 border-white/10 text-white font-black text-sm px-3 py-1">
                  <Timer className="size-4 mr-1.5 text-primary" />{" "}
                  {colorTimeLeft}с
                </Badge>
              ...
              {colorTimeLeft > 0 ? (
                <>
                  ...
                  <div
                    className={cn(
                      "text-5xl sm:text-6xl font-black uppercase",
                      colorWord.displayColor,
                    )}
                  >
                    {colorWord.text}
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {colorOptions.map((opt) => (
                      <Button
                        key={opt}
                        className="h-14 font-black text-sm rounded-xl bg-white/5 border border-white/10 hover:bg-primary/20 transition-all active:scale-95"
                ...
                <div className="space-y-4 py-8">
                  ...
                  <Button
                    className="cyber-gradient rounded-xl h-12 px-8 font-black"
          ...
          >
            <Card className="glass-panel max-w-lg mx-auto p-8 sm:p-12 text-center space-y-8 border-0 rounded-[2.5rem]">
              ...
              <div className="flex gap-2 max-w-xs mx-auto">
                ...
                />
                <Button
                  className="cyber-gradient rounded-xl h-12 px-5 font-black"
                  onClick={checkWord}
                >
                  OK
                </Button>
            ...
            <Card className="glass-panel max-w-lg mx-auto p-8 sm:p-12 text-center space-y-8 border-0 rounded-[2.5rem]">
              ...
              <div className="space-y-2">
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                  Дилер {bjGameOver ? `• ${sumCards(bjDealerCards)}` : ""}
                </p>
                <div className="flex items-center justify-center gap-2">
                  {bjDealerCards.map((c, i) => (
                    <div
                      key={i}
                      className="size-14 sm:size-16 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center justify-center text-xl font-black text-red-400"
                    >
                      {c}
                    </div>
                  ))}
                  {!bjGameOver && (
                    <div className="size-14 sm:size-16 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-xl font-black text-white/20">
                      ?
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center justify-center gap-2 flex-wrap">
                  {bjCards.map((c, i) => (
                    <motion.div
                      ...
                      animate={{ scale: 1, rotate: 0 }}
                      className={cn(
                        "size-14 sm:size-16 rounded-xl border flex items-center justify-center text-xl font-black",
                        sumCards(bjCards) > 21
              ...
              {!bjGameOver ? (
                <div className="flex gap-3 justify-center">
                  <Button
                    className="h-14 px-8 rounded-xl bg-white/5 border border-white/10 font-black text-base hover:bg-primary/20 transition-all"
                    onClick={bjHit}
                  >
                    Ще
                  </Button>
                  <Button
                    className="h-14 px-8 rounded-xl cyber-gradient font-black text-base"
                    onClick={bjStand}
                  >
                    Стоп
                  </Button>
...
function GameCard({
  ...
  return (
    <div
      className={cn(
        "glass-panel flex flex-col p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] h-full group overflow-hidden transition-all",
        disabled && "opacity-50 grayscale",
      )}
    >
      <div className="size-12 sm:size-14 rounded-xl bg-white/5 border border-white/[0.06] flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform">
        {icon}
      </div>
      <h3 className="font-headline text-lg sm:text-xl text-white font-bold mb-1.5 uppercase">
        {title}
      </h3>
      <p className="text-muted-foreground mb-4 sm:mb-6 text-xs leading-relaxed flex-1 opacity-70">
        {desc}
      </p>
      <div className="flex flex-wrap gap-1.5 mb-4 sm:mb-6">
        {tags.map((tag) => (
          <Badge
            key={tag}
            variant="outline"
            className="text-[8px] sm:text-[9px] uppercase font-black px-2 bg-white/[0.03] border-white/[0.06]"
          >
            {tag}
          </Badge>
        ))}
      ...
      <Button
        disabled={disabled}
        className="w-full h-11 sm:h-12 rounded-xl cyber-gradient text-white font-black uppercase tracking-widest text-xs"
        onClick={onPlay}
      >
