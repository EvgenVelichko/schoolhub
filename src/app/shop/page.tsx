...
const shopItems: ShopItem[] = [
  ...
  },
  {
    ...
    price: 6000,
    icon: <Palette className="size-6 text-fuchsia-400" />,
    category: "cosmetic",
    color: "fuchsia",
    rarity: "epic",
  },
  {
    ...
    price: 2000,
    icon: <Bolt className="size-6 text-amber-400" />,
    badge: "Boost",
    category: "boost",
    color: "amber",
    rarity: "rare",
  },
  {
    ...
    price: 5500,
    icon: <Snowflake className="size-6 text-sky-300" />,
    badge: "FX",
    category: "effect",
    color: "sky",
    rarity: "rare",
  },
  {
    ...
    price: 1800,
    icon: <Music className="size-6 text-violet-400" />,
    category: "cosmetic",
    color: "violet",
    rarity: "common",
  },
  {
    ...
    price: 3000,
    icon: <Eye className="size-6 text-teal-400" />,
    badge: "Utility",
    category: "boost",
    color: "teal",
    rarity: "rare",
  },
  {
    ...
    price: 4500,
    icon: <Swords className="size-6 text-rose-500" />,
    category: "cosmetic",
    color: "rose",
    rarity: "epic",
  },
  {
    ...
    description: "Ігровий бар здоров'я під аватаром. Чим вищий бал — тим більше HP.",
    price: 3200,
    icon: <HeartPulse className="size-6 text-red-400" />,
    badge: "FX",
    category: "effect",
    color: "red",
    rarity: "rare",
  ...
  },
  {
    ...
    price: 9000,
    icon: <Wand2 className="size-6 text-purple-300" />,
    badge: "FX",
    category: "effect",
    color: "purple",
    rarity: "epic",
  },
  {
    ...
    price: 4000,
    icon: <Cloudy className="size-6 text-blue-300" />,
    badge: "FX",
    category: "effect",
    color: "blue",
    rarity: "rare",
...
export default function ShopPage() {
  ...
  const [activeCategory, setActiveCategory] = React.useState<ShopCategory>("all")
  ...
  const handleBuy = async (item: ShopItem) => {
    ...
    try {
      ...
      if (purchasedCount >= 5 && !achievements.includes('shopaholic')) achievements.push('shopaholic')
      if (item.id === 'matrix_effect' && !achievements.includes('matrix_hacker')) achievements.push('matrix_hacker')
      if (item.id === 'ghost_mode' && !achievements.includes('ghost_walker')) achievements.push('ghost_walker')
  ...
  return (
    <div className="p-3 sm:p-4 md:p-8 max-w-5xl mx-auto space-y-6 sm:space-y-8 animate-reveal pb-32">
      ...
      <header className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 sm:gap-4">
            <Button variant="ghost" size="icon" className="size-10 sm:size-12 rounded-xl bg-white/5 hover:bg-white/10" onClick={() => router.back()}>
      ...
      {/* Featured item — first legendary */}
      {activeCategory === "all" && (() => {
        const featured = shopItems.find(i => i.rarity === "legendary" && !profile?.purchasedItems?.includes(i.id))
          ...
          >
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-5 sm:gap-8">
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3 className="text-xl sm:text-2xl font-bold text-white">{featured.title}</h3>
                <p className="text-muted-foreground text-xs sm:text-sm mt-1 opacity-70">{featured.description}</p>
              </div>
              <div className="flex items-center gap-3 sm:gap-4 shrink-0 w-full sm:w-auto">
                <div className="flex items-center gap-1.5">
                  <Coins className="size-5 text-yellow-500" />
                  <span className="text-2xl font-black text-white">{isOwner ? 0 : featured.price}</span>
                </div>
                <Button
                  className={cn("cyber-gradient rounded-xl h-11 px-6 font-black text-xs flex-1 sm:flex-none", !canAfford && "opacity-50")}
        ...
        <AnimatePresence mode="popLayout">
          {filtered.map((item, i) => {
            ...
            return (
              ...
              >
                {/* Rarity gradient top strip */}
                <div className={cn("h-0.5 w-full bg-gradient-to-r", RARITY_COLORS[item.rarity].replace("border-", "from-").replace("/20", "/40"))} />
                ...
                <div className="p-4 sm:p-5 flex flex-col h-full">
                    ...
                    <div className="flex flex-col items-end gap-1.5">
                      {isOwned && (
                        <div className="flex items-center gap-1 bg-green-500/10 text-green-400 px-2 py-0.5 rounded-md">
                          ...
                          <span className="text-[8px] font-black uppercase">Є</span>
                        </div>
                      )}
                      <span className={cn("text-[8px] font-black uppercase tracking-widest", RARITY_TEXT[item.rarity])}>
                  ...
                  {/* Title + description */}
                  <h3 className="text-base sm:text-lg font-bold text-white leading-tight mb-1">{item.title}</h3>
                  <p className="text-muted-foreground text-[11px] sm:text-xs leading-relaxed flex-1 opacity-70">{item.description}</p>
                  ...
                  <div className="mt-4 pt-3 border-t border-white/[0.04] flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={cn(
                        ...
                        isOwned ? "text-white/40 line-through" : canAfford ? "text-white" : "text-red-400"
                      )}>
                        {currentPrice}
                      </span>
                    </div>
                    ...
                    {isOwned ? (
                      <span className="text-[9px] font-black uppercase tracking-widest text-green-400/60">Придбано</span>
                    ) : (
                      <Button
                        size="sm"
                        className={cn(
                          "rounded-xl h-8 sm:h-9 px-3 sm:px-5 font-black text-[9px] sm:text-[10px] uppercase tracking-wider",
                          canAfford ? "cyber-gradient" : "bg-white/5 text-white/40"
                        )}
                        disabled={!canAfford || isBuying}
                        onClick={() => handleBuy(item)}
                      >
                        {isBuying ? "..." : !canAfford ? <><Lock className="size-3 mr-1" /> Мало</> : "Купити"}
                      </Button>
                    )}
                  </div>
                </div>
