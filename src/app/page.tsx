  ...
  const performSync = async (manual = false) => {
    ...
    try {
      ...
      if (result && result.success && result.data) {
        ...
        // New achievements
        if (!achievements.includes('first_sync')) achievements.push('first_sync');
        if (avg >= 11 && scoreCount > 0 && !achievements.includes('top_student')) achievements.push('top_student');
        if (scoreCount >= 100 && !achievements.includes('bookworm')) achievements.push('bookworm');
        if ((profile?.purchasedItems?.length || 0) >= 5 && !achievements.includes('shopaholic')) achievements.push('shopaholic');
        if (profile?.purchasedItems?.includes('matrix_effect') && !achievements.includes('matrix_hacker')) achievements.push('matrix_hacker');
        if (profile?.purchasedItems?.includes('ghost_mode') && !achievements.includes('ghost_walker')) achievements.push('ghost_walker');
        ...
        <header className="flex flex-col sm:flex-row justify-between items-center glass-panel p-4 sm:p-6 md:p-8 rounded-[2rem] sm:rounded-[2.5rem] border-0 relative overflow-hidden gap-4 sm:gap-6">
          <div className="flex items-center gap-3 sm:gap-4 md:gap-6 z-10 w-full sm:w-auto">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-lg sm:text-xl md:text-3xl font-headline font-bold text-white truncate leading-tight">Привіт, {profile?.displayName?.split(' ')[0]}!</h1>
              <div className="flex items-center gap-1.5 sm:gap-2 mt-1.5 sm:mt-2 flex-wrap">
                <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20 text-[9px] sm:text-[10px] font-black uppercase tracking-widest px-2 sm:px-3 py-0.5 sm:py-1">
                  {(profile?.gradeLevel || "Школяр").toString().trim()}
                </Badge>
                {profile?.achievements?.includes('night_owl') && <Star className="size-3.5 sm:size-4 text-indigo-400" />}
                {profile?.achievements?.includes('smart_lazy') && <Zap className="size-3.5 sm:size-4 text-emerald-400" />}
                {profile?.achievements?.includes('rich_student') && <Coins className="size-3.5 sm:size-4 text-yellow-500" />}
              </div>
            </div>
          </div>
          <div className="flex items-center justify-between sm:justify-end gap-3 sm:gap-4 z-10 w-full sm:w-auto">
            <div className="flex items-center gap-2 sm:gap-3 text-yellow-500 bg-yellow-500/10 px-4 sm:px-6 py-2 sm:py-3 rounded-xl sm:rounded-2xl border border-yellow-500/20 shrink-0">
              ...
              <span className="text-base sm:text-xl font-black">{isOwner ? "∞" : (profile?.coins || 0)}</span>
        </div>
            <Button variant="ghost" size="icon" className={cn("size-11 sm:size-14 rounded-xl sm:rounded-2xl bg-white/5", isSyncing && "animate-spin text-primary")} onClick={handleManualSync} disabled={isSyncing}>
          ...
          <div className="flex items-center justify-between px-1 sm:px-2 gap-2">
             <div className="flex items-center gap-2 sm:gap-4 min-w-0">
               ...
               <div className="min-w-0">
                 <h2 className="text-base sm:text-xl md:text-2xl font-headline font-bold text-white truncate">{scheduleLabel} • {format(targetDate, 'd MMM', { locale: uk })}</h2>
               </div>
             </div>
             <Button variant="ghost" asChild className="text-primary font-black uppercase text-[9px] sm:text-[10px] tracking-widest hover:bg-primary/5 rounded-xl shrink-0 px-2 sm:px-4">
               <Link href="/schedule">Тиждень <ArrowRight className="ml-1 sm:ml-2 size-3.5 sm:size-4" /></Link>
             </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
          <div className="glass-panel p-6 sm:p-10 border-0 rounded-[2rem] sm:rounded-[3.5rem] flex flex-col items-center justify-center text-center gap-4 sm:gap-8 relative overflow-hidden group">
            ...
            <div className="w-full space-y-3 sm:space-y-4 z-10">
              ...
              <p className="text-[8px] sm:text-[9px] text-muted-foreground italic leading-relaxed">Ваш рівень — це ваш середній бал.</p>
                         </div>
          ...
          <Card className="glass-panel border-0 rounded-[2rem] sm:rounded-[3.5rem] md:col-span-2 overflow-hidden flex flex-col">
            <CardHeader className="p-4 sm:p-6 md:p-8 bg-white/[0.02] border-b border-white/5 flex flex-row items-center justify-between">
              ...
              <Button variant="ghost" asChild className="text-primary font-bold text-[10px] sm:text-xs hover:bg-primary/5 rounded-xl px-2 sm:px-4">
                <Link href="/grades">Журнал <ArrowRight className="ml-1 sm:ml-2 size-3.5 sm:size-4" /></Link>
                           </Button>
            </CardHeader>
              ...
              {latestGrades?.length ? latestGrades.map((g: any, i: number) => (
                <div key={i} className="p-3 sm:p-5 md:p-6 flex items-center justify-between hover:bg-white/[0.02] transition-colors gap-2">
                  <div className="flex items-center gap-2.5 sm:gap-4 md:gap-5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-white text-xs sm:text-sm md:text-base truncate">{g.subject}</p>
                      <p className="text-[9px] sm:text-[10px] text-muted-foreground uppercase font-black tracking-widest mt-0.5 sm:mt-1 opacity-60 truncate">{g.type}</p>
                    </div>
                  </div>
                  <span className="text-[8px] sm:text-[9px] font-bold text-muted-foreground bg-white/5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl border border-white/10 shrink-0">{g.date}</span>
                        </div>
                      )) : (
                <div className="p-12 sm:p-24 text-center opacity-30 text-sm sm:text-base font-medium">Очікування синхронізації...</div>
                      )}
                   </div>
          </Card>
        </div>
