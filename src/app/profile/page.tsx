...
export default function ProfilePage() {
  ...
  if (!user || !profile) {
    return (
      <div className="h-[70vh] flex flex-col items-center justify-center gap-4 p-6 text-center text-white">
        ...
        <Button
          className="cyber-gradient border-0"
  ...
  const hasRainbowAura =
    profile?.purchasedItems?.includes("aura_rainbow") || isOwner;
  ...
  const hasProfessor =
    profile?.purchasedItems?.includes("professor_badge") || isOwner;
  const hasDiamond =
    profile?.purchasedItems?.includes("diamond_badge") || isOwner;
  const hasMatrix =
    profile?.purchasedItems?.includes("matrix_effect") || isOwner;
  const hasGhost = profile?.purchasedItems?.includes("ghost_mode") || isOwner;
  const hasGoldenFrame =
    profile?.purchasedItems?.includes("golden_frame") || isOwner;
  const hasSnow = profile?.purchasedItems?.includes("snow_effect") || isOwner;
  const hasMusicBadge =
    profile?.purchasedItems?.includes("music_badge") || isOwner;
  const hasBattleTitle =
    profile?.purchasedItems?.includes("battle_title") || isOwner;
  const hasHpBar = profile?.purchasedItems?.includes("hp_bar") || isOwner;
  const hasChampionCrown =
    profile?.purchasedItems?.includes("champion_crown") || isOwner;
  const hasMagicParticles =
    profile?.purchasedItems?.includes("magic_particles") || isOwner;
  const hasCloudBg = profile?.purchasedItems?.includes("cloud_bg") || isOwner;
  const hasCustomTheme =
    profile?.purchasedItems?.includes("custom_theme") || isOwner;
  const hasStealthMode =
    profile?.purchasedItems?.includes("stealth_mode") || isOwner;
        ...
        >
          ...
          {hasFire && !hasMatrix && (
            <div className="absolute inset-0 flex items-center justify-center opacity-20">
              <Flame className="size-32 sm:size-48 md:size-64 text-white animate-pulse" />
            </div>
          )}
          {hasSnow && <SnowEffect />}
          ...
          <div className="absolute top-3 right-3 sm:top-6 sm:right-6 flex gap-2 z-20">
            <Button
              variant="secondary"
              className="bg-white/10 hover:bg-white/20 border-white/10 text-white backdrop-blur-md rounded-xl sm:rounded-2xl h-9 sm:h-12 px-3 sm:px-6 font-bold text-xs sm:text-sm"
              onClick={() => setIsEditing(!isEditing)}
            >
              <Edit3 className="size-3.5 sm:size-4 mr-1.5 sm:mr-2" />{" "}
        ...
        <div className="px-4 sm:px-6 md:px-12 -mt-16 sm:-mt-20 md:-mt-24 relative z-10 flex flex-col md:flex-row items-center md:items-end gap-4 sm:gap-8 text-center md:text-left">
          <div
            className={cn(
              ...
              hasGhost && "ghost-shimmer",
              hasGoldenFrame && "golden-frame rounded-full",
            )}
          >
            <Avatar
              className={cn(
                ...
                hasAura &&
                  "ring-4 ring-yellow-500 ring-offset-4 ring-offset-[#0c0805]",
                hasRainbowAura && "aura-rainbow",
                hasGhost && "opacity-80",
              )}
            >
              <AvatarImage src={profile.photoURL} className="object-cover" />
              <AvatarFallback className="text-4xl sm:text-6xl bg-primary/20 text-primary font-bold">
            ...
            {hasHpBar && (
              <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 w-[80%] hp-bar">
                <div
                  ...
                  style={{
                    width: `${Math.min(((profile.level || 1) / 12) * 100, 100)}%`,
                  }}
                />
              </div>
            )}
          ...
          <div className="flex-1 pb-2 sm:pb-4 md:pb-6 space-y-3 sm:space-y-4 min-w-0">
            <div className="flex flex-col md:flex-row md:items-center gap-2 sm:gap-4 justify-center md:justify-start">
              ...
              <div className="flex items-center gap-2 sm:gap-3 justify-center md:justify-start flex-wrap">
                {hasChampionCrown && (
                  <Crown className="size-8 sm:size-12 text-yellow-400 champion-crown" />
                )}
                {(hasVIP || isOwner) && !hasChampionCrown && (
                  <Crown className="size-7 sm:size-10 text-yellow-500 drop-shadow-[0_0_15px_rgba(234,179,8,0.6)]" />
                )}
                {hasProfessor && (
                  <GraduationCap className="size-7 sm:size-10 text-emerald-400 drop-shadow-[0_0_15px_rgba(52,211,153,0.6)]" />
                )}
                {hasDiamond && (
                  <Gem className="size-7 sm:size-10 text-cyan-400 drop-shadow-[0_0_15px_rgba(34,211,238,0.6)]" />
                )}
                {hasMusicBadge && (
                  <Music className="size-6 sm:size-8 text-violet-400 music-note" />
                )}
                {hasGhost && (
                  <Ghost className="size-7 sm:size-10 text-slate-400 drop-shadow-[0_0_15px_rgba(148,163,184,0.4)]" />
                )}
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 sm:gap-4">
              <Badge
                className={cn(
                  "border-0 px-4 sm:px-6 py-1.5 sm:py-2 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-xl",
                  isOwner
              ...
              <div className="flex items-center gap-2 sm:gap-4 bg-white/5 border border-white/10 px-3 sm:px-6 py-1.5 sm:py-2 rounded-xl backdrop-blur-md">
                <span className="text-white font-black uppercase text-[9px] sm:text-[10px] tracking-widest">
                  LV {profile.level}
                </span>
                <div className="size-1 sm:size-1.5 bg-white/20 rounded-full" />
                <span className="text-primary font-black uppercase text-[9px] sm:text-[10px] tracking-widest">
                  {profile.xp || 0} XP
                </span>
              </div>
              {hasBoost && (
                <Badge className="bg-yellow-500/20 text-yellow-500 border-yellow-500/30 gap-1.5 sm:gap-2 h-8 sm:h-10 px-3 sm:px-4 rounded-xl font-black uppercase text-[8px] sm:text-[9px]">
                  <Zap className="size-3 sm:size-3.5" /> 2X XP
                </Badge>
              )}
              {hasBattleTitle && (
                <Badge className="bg-rose-500/20 text-rose-400 border-rose-500/30 gap-1.5 sm:gap-2 h-8 sm:h-10 px-3 sm:px-4 rounded-xl font-black uppercase text-[8px] sm:text-[9px]">
                  <Swords className="size-3 sm:size-3.5" /> ВОЇН
                </Badge>
              )}
              {hasStealthMode && (
                <Badge className="bg-teal-500/20 text-teal-400 border-teal-500/30 gap-1.5 sm:gap-2 h-8 sm:h-10 px-3 sm:px-4 rounded-xl font-black uppercase text-[8px] sm:text-[9px]">
                  Офлайн
                </Badge>
              )}
            </div>
        ...
        <div className="space-y-4 sm:space-y-6">
          <Card className="glass-panel border-0 rounded-[2rem] sm:rounded-[2.5rem] p-3 sm:p-4">
            <CardHeader className="p-3 sm:p-6">
              <CardTitle className="text-lg sm:text-xl flex items-center gap-3 text-white">
                <Trophy className="size-5 sm:size-6 text-primary" /> Статистика
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2 sm:space-y-3 p-3 sm:p-6 pt-0">
              ...
              <StatItem
                ...
                label="Монети"
                value={isOwner ? "∞" : profile.coins || 0}
              />
              <StatItem
                icon={<Star className="text-primary" />}
                label="Статус"
                value={
                  isOwner ? "Founder" : profile.academicStatus || "Новачок"
                }
              />
              <StatItem
          ...
          {isEditing ? (
            <Card className="glass-panel border-0 animate-in slide-in-from-top-4 rounded-[2rem] sm:rounded-[2.5rem]">
              <CardHeader className="p-6 sm:p-10 pb-4">
                ...
                <CardDescription>
                  Змініть те, як інші бачать ваш профіль у School Hub.
                </CardDescription>
              </CardHeader>
              <CardContent className="p-6 sm:p-10 pt-4 space-y-4 sm:space-y-6">
                <div className="space-y-2 sm:space-y-3">
                  ...
                  <Input
                    ...
                    className="bg-white/5 border-white/10 h-12 sm:h-16 rounded-xl sm:rounded-2xl text-base sm:text-lg focus:ring-primary/40 px-4 sm:px-6 text-white"
                  />
                </div>
                <div className="space-y-2 sm:space-y-3">
                  <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] ml-1">
                  ...
                  <Textarea
                    ...
                    className="bg-white/5 border-white/10 min-h-[140px] sm:min-h-[180px] rounded-[1.5rem] sm:rounded-[2rem] text-white p-5 sm:p-8 text-base sm:text-lg focus:ring-primary/40 leading-relaxed"
                    placeholder="Розкажіть про свої захоплення та досягнення..."
                  />
                </div>
                <div className="flex gap-3 sm:gap-4 justify-end pt-4 sm:pt-6">
                  <Button
                    variant="ghost"
                    className="rounded-xl sm:rounded-2xl h-11 sm:h-14 px-6 sm:px-10 font-bold text-sm"
                    onClick={() => setIsEditing(false)}
                  >
                    Скасувати
                  </Button>
                  <Button
                    className="cyber-gradient border-0 rounded-xl sm:rounded-2xl h-11 sm:h-14 px-8 sm:px-16 font-black uppercase text-[10px] sm:text-xs tracking-widest"
                    ...
                    <div className="flex flex-wrap gap-2">
                      {purchasedItems.map((itemId: string) => (
                        <Badge
                          key={itemId}
                          variant="outline"
                          className="bg-white/5 border-white/10 text-white/70 text-[9px] sm:text-[10px] px-3 py-1 rounded-lg"
                        >
                          {itemId.replace(/_/g, " ")}
                        </Badge>
                      ))}
...
function StatItem({
  ...
  return (
    <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.06] transition-all group shadow-inner">
      ...
      <span className="font-black text-sm sm:text-lg text-white italic drop-shadow-md ml-auto text-right truncate">
        {value}
      </span>
    </div>
  );
