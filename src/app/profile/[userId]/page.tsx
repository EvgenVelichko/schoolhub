...
function SnowEffect() {
  const flakes = React.useMemo(() =>
    Array.from({ length: 20 }, (_, i) => ({
      id: i, left: `${Math.random() * 100}%`, delay: `${Math.random() * 5}s`, duration: `${4 + Math.random() * 6}s`, size: `${8 + Math.random() * 8}px`,
    })), [])
  return (<div className="snow-container">{flakes.map(f => (<div key={f.id} className="snowflake" style={{ left: f.left, animationDelay: f.delay, animationDuration: f.duration, fontSize: f.size }}>❄</div>))}</div>)
}

function MagicParticles() {
  const particles = React.useMemo(() =>
    Array.from({ length: 15 }, (_, i) => ({
      id: i, left: `${10 + Math.random() * 80}%`, top: `${10 + Math.random() * 80}%`, delay: `${Math.random() * 4}s`, duration: `${2 + Math.random() * 3}s`, size: `${3 + Math.random() * 4}px`,
    })), [])
  return (<div className="particles-container">{particles.map(p => (<div key={p.id} className="particle" style={{ left: p.left, top: p.top, animationDelay: p.delay, animationDuration: p.duration, width: p.size, height: p.size }} />))}</div>)
}

function CloudBg() {
  const clouds = React.useMemo(() =>
    Array.from({ length: 5 }, (_, i) => ({
      id: i, top: `${10 + Math.random() * 60}%`, width: `${120 + Math.random() * 160}px`, height: `${60 + Math.random() * 80}px`, delay: `${i * 3 + Math.random() * 2}s`, duration: `${15 + Math.random() * 10}s`,
    })), [])
  return (<div className="cloud-container">{clouds.map(c => (<div key={c.id} className="cloud" style={{ top: c.top, width: c.width, height: c.height, animationDelay: c.delay, animationDuration: c.duration }} />))}</div>)
}
...
export default function UserProfilePage() {
  ...
  const userDocRef = React.useMemo(() => (userId ? doc(db, "users", userId as string) : null), [db, userId])
  ...
  if (!profile) return <div className="p-6 text-center text-white">Користувача не знайдено.</div>
        ...
        <div className="space-y-4 sm:space-y-6">
           ...
           {!isOwnProfile && currentUser && (
             <div className="flex flex-col gap-3">
                <Button variant="outline" className="bg-white/5 border-white/10 h-14 sm:h-16 rounded-2xl font-bold gap-3 text-sm" onClick={() => router.push(`/messages?with=${userId}`)}>
                  <MessageSquare className="size-5" /> Надіслати повідомлення
                </Button>
             </div>
           )}
        </div>
