...
export default function UserProfilePage() {
  ...
  const friendsQuery = React.useMemo(() => (userId ? collection(db, "users", userId as string, "friends") : null), [db, userId])
        ...
        <div className="space-y-6">
           ...
           {!isOwnProfile && currentUser && (
             <div className="flex flex-col gap-3">
                <Button className="cyber-gradient h-16 rounded-2xl font-bold gap-3 shadow-xl">
                  <UserPlus className="size-5" /> Додати в друзі
                </Button>
                <Button variant="outline" className="bg-white/5 border-white/10 h-16 rounded-2xl font-bold gap-3" onClick={() => router.push(`/messages?with=${userId}`)}>
                ...
                <TabsContent value="friends" className="pt-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {friends && friends.length > 0 ? friends.map((friend: any) => (
                        <div key={friend.id} className="glass-panel p-6 rounded-[2rem] flex items-center gap-5 group hover:border-primary/50 transition-all shadow-xl" onClick={() => router.push(`/profile/${friend.id}`)}>
                           ...
                           <Button variant="ghost" size="icon" className="size-12 rounded-2xl hover:bg-primary/20 shrink-0 text-primary" onClick={(e) => { e.stopPropagation(); router.push(`/messages?with=${friend.id}`); }}>
                        ...
                        <Card className="glass-panel border-0 col-span-2 py-32 text-center rounded-[3.5rem] shadow-2xl">
                           ...
                           <p className="text-muted-foreground font-bold text-lg">У користувача поки немає друзів.</p>
                        </Card>
                      )}
                   </div>
                </TabsContent>
