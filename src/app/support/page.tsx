...
export default function SupportChat() {
  ...
  const reportId = React.useMemo(() => (user ? `report-${user.uid}` : null), [user])
  const reportRef = React.useMemo(() => (reportId ? doc(db, "reports", reportId) : null), [db, reportId])
  const { data: report, loading: reportLoading } = useDoc(reportRef)

  const messagesQuery = React.useMemo(() => (reportId ? query(
      ...
      <header className="mobile-header-blur p-4 flex items-center justify-between shrink-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-base font-headline font-bold text-white truncate">Підтримка</h1>
            <p className="text-[10px] text-muted-foreground uppercase font-black tracking-widest opacity-60">Reports v2</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {report && (
            <Button variant="ghost" size="icon" className="size-10 rounded-xl text-destructive hover:bg-destructive/10" onClick={handleClearAndClose}>
      ...
      <CardContent className="flex-1 overflow-y-auto p-4 md:p-8 flex flex-col-reverse gap-6 scrollbar-none overscroll-contain bg-gradient-to-b from-transparent to-[#0a0512] pb-32">
          ...
          messages?.map((msg: any, i: number) => {
            ...
            return (
              <motion.div 
                ...
                className={cn("flex gap-3 max-w-[85%]", isOwn ? 'ml-auto flex-row-reverse' : 'mr-auto')}
              >
                {!isOwn && (
                  <Avatar className="size-9 border border-white/10 shrink-0 mt-auto shadow-md">
                    <AvatarFallback className="text-[9px] font-bold bg-white/5 text-white">A</AvatarFallback>
                  </Avatar>
                )}
        ...
        {!messages?.length && !reportLoading && (
          <div className="h-full flex flex-col items-center justify-center text-muted-foreground opacity-20 space-y-6">
            ...
            <p className="text-lg font-headline font-bold uppercase tracking-widest text-center px-10">Маєте запитання? Ми тут, щоб допомогти!</p>
          </div>
        )}
      </CardContent>
