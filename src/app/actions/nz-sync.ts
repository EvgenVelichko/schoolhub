...
function parseJson(raw: string, tag: string): any {
  ...
  if (t[0] === "<") {
    ...
    throw new Error(
      `NZ.ua повернув HTML замість даних (${tag}). Можливо, невірний логін/пароль або сервер тимчасово недоступний`,
    );
  }
  try {
    return JSON.parse(t);
  } catch {
    throw new Error(`Невалідний JSON (${tag})`);
  }
}
...
export async function syncWithNzPortal(
  login: string,
  pass: string,
  opts?: { deep?: boolean },
) {
  ...
  try {
    ...
    for (const day of diary.dates || []) {
      ...
      for (const call of day.calls || []) {
        ...
        const timeStart: string =
          call.call_time_start || call.time_start || "--:--";
        ...
        for (const subj of call.subjects || []) {
          ...
          const lessonArr: any[] = Array.isArray(subj.lesson)
            ? subj.lesson
            : subj.lesson
              ? [subj.lesson]
              : [];
          ...
          for (let li = 0; li < lessonArr.length; li++) {
            ...
            const mark: string = (
              typeof l.mark === "string" ? l.mark : String(l.mark ?? "")
            ).trim();
            if (!mark) continue;
            ...
            if (!gradeKeys.has(key)) {
              ...
              grades.push({
                ...
                date,
                type,
                timestamp: dateTs(date) + callNum * 100 + li,
                sourceKey: key,
              });
            }
          }
          ...
          for (const l of lessonArr) {
            if (l.comment) {
              const t = textOf(l.comment);
              if (t) hwParts.push(t);
            }
          }

          const teacherName =
            typeof subj.teacher === "object" ? subj.teacher?.name : "";
      ...
      if (perf.subjects && Array.isArray(perf.subjects)) {
        for (const s of perf.subjects) {
          ...
          for (let i = 0; i < s.marks.length; i++) {
            ...
            const mark: string = (
              typeof m === "object" ? m.value : String(m)
            ).trim();
            const type: string =
              typeof m === "object" ? m.type || "Поточна" : "Поточна";
            ...
            const existingForSubject = grades.filter(
              (g) => g.subject === name,
            ).length;
    ...
    if (opts?.deep && subjectIdMap.size > 0) {
      let deepAdded = 0;

      // Fetch all subjects in parallel (batches of 5 to avoid rate limiting)
      const entries = [...subjectIdMap.entries()];
      const BATCH = 5;

      for (let b = 0; b < entries.length; b += BATCH) {
        const batch = entries.slice(b, b + BATCH);
        const results = await Promise.allSettled(
          batch.map(([rawName, subjectId]) =>
            post(
