...
export default function KnowledgeHub() {
  ...
  const threadsQuery = React.useMemo(
  ...
  const handleCreateThread = async () => {
    if (!user)
      return toast({ title: "Потрібна авторизація", variant: "destructive" });
    if (profile?.isMuted)
      return toast({ title: "Ви замучені", variant: "destructive" });
    if (!newThreadTitle || !newThreadContent)
      return toast({ title: "Заповніть поля", variant: "destructive" });
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
          ...
          <div className="space-y-1 min-w-0">
            ...
            <p className="text-muted-foreground text-xs sm:text-sm opacity-70">
              Спілкуйся та навчайся разом з усіма
            </p>
          </div>
        </div>
        <div className="w-full sm:w-auto">
          {profile?.isMuted ? (
            <Badge
              variant="destructive"
              className="h-11 sm:h-12 px-6 sm:px-8 flex items-center gap-3 rounded-xl sm:rounded-2xl w-full sm:w-auto justify-center font-black text-xs uppercase tracking-widest"
            >
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="glass-panel border-0 sm:max-w-[550px] rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10">
                <DialogHeader>
                  <DialogTitle className="text-2xl sm:text-3xl font-headline font-bold text-white text-center">
                    Створити гілку
                  </DialogTitle>
                </DialogHeader>
                <div className="space-y-5 sm:space-y-6 py-4 sm:py-6">
                  <div className="space-y-2">
                    ...
                    <Input
                      placeholder="Як розв'язати це рівняння?"
                      value={newThreadTitle}
                      onChange={(e) => setNewThreadTitle(e.target.value)}
                      className="bg-white/5 border-white/10 h-12 sm:h-14 rounded-xl sm:rounded-2xl px-4 sm:px-6 text-white"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">
                    ...
                    <Select
                      ...
                      onValueChange={setNewThreadSubject}
                    >
                        ...
                        {subjects
                          ...
                          .map((s) => (
                            <SelectItem
                              key={s}
                              value={s}
                              className="rounded-lg sm:rounded-xl focus:bg-primary/20"
                            >
                              {s}
                            </SelectItem>
                          ))}
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      placeholder="Опишіть ваше запитання детальніше..."
                      className="bg-white/5 border-white/10 min-h-[120px] sm:min-h-[150px] rounded-xl sm:rounded-2xl p-4 sm:p-6 text-white"
                ...
                <DialogFooter className="gap-3">
                  <Button
                    variant="ghost"
                    className="h-11 sm:h-14 rounded-xl sm:rounded-2xl px-6 sm:px-8 hover:bg-white/5 font-bold text-sm"
                    onClick={() => setIsDialogOpen(false)}
                  >
                    Скасувати
                  </Button>
                  <Button
                    className="cyber-gradient h-11 sm:h-14 rounded-xl sm:rounded-2xl px-8 sm:px-12 font-black text-xs uppercase tracking-widest"
      ...
      <div className="space-y-3 sm:space-y-4 md:space-y-6">
        {threadsLoading ? (
          ...
          filteredThreads.map((thread: any) => (
            <Card
              key={thread.id}
              className="glass-panel hover:border-primary/30 transition-all group overflow-hidden border-0 rounded-[1.5rem] sm:rounded-[2rem]"
            >
              <CardHeader className="p-4 sm:p-6 md:p-8 pb-2 sm:pb-4 flex flex-row items-center justify-between gap-3">
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  <Avatar className="size-10 sm:size-12 border border-primary/20 shrink-0">
                    <AvatarImage
                      src={`https://picsum.photos/seed/${thread.id}/100`}
                    />
                ...
                <Badge className="bg-yellow-500/10 text-yellow-500 border-yellow-500/20 rounded-lg sm:rounded-xl px-2.5 sm:px-4 py-1 text-[9px] sm:text-[10px] font-black shrink-0">
                  +{thread.reward} XP
                </Badge>
              </CardHeader>
              <CardContent className="p-4 sm:p-6 md:p-8 pt-1 sm:pt-2 space-y-3 sm:space-y-4 md:space-y-6">
                <h3 className="text-base sm:text-xl md:text-2xl font-headline font-bold text-white group-hover:text-primary transition-colors leading-tight">
                  {thread.title}
                </h3>
                ...
                <div className="flex items-center gap-4 sm:gap-8 pt-3 sm:pt-6 border-t border-white/5 text-xs sm:text-sm font-bold text-muted-foreground">
                  ...
                  <button className="flex items-center gap-1.5 sm:gap-2 hover:text-pink-500 transition-colors">
          ...
          <div className="glass-panel border-0 rounded-[2rem] sm:rounded-[2.5rem] py-16 sm:py-24 text-center text-muted-foreground space-y-4">
            ...
            <p className="text-base sm:text-xl font-bold italic">
              Поки що пусто... Створіть першу тему!
            </p>
          </div>
        )}
      </div>
