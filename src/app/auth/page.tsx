...
export default function AuthPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      toast({ title: "Помилка входу", description: e.message, variant: "destructive" })
    }
  }

  const handleEmailAction = async (action: 'login' | 'register') => {
    if (!email || !password || (action === 'register' && !name)) {
      toast({ title: "Увага", description: "Заповніть всі необхідні поля", variant: "destructive" })
      return
    }
    setIsPending(true)
    try {
        ...
        <Card className="glass-panel border-white/5 border-0 rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden">
          <Tabs defaultValue="login" className="w-full">
            ...
            <CardContent className="p-5 sm:p-10">
              <TabsContent value="login" className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-500">
                <div className="space-y-5">
                  <div className="space-y-2">
                    <Label htmlFor="email" className="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1">Електронна пошта</Label>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center ml-1">
                      <Label htmlFor="password" className="text-xs font-black uppercase tracking-widest text-muted-foreground">Пароль</Label>
                      <button className="text-[10px] font-bold text-primary hover:underline">Забули пароль?</button>
                    </div>
                    <div className="relative group">
              ...
              <TabsContent value="register" className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                <div className="space-y-5">
                  <div className="space-y-2">
                    <Label htmlFor="name" className="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1">Повне ім'я</Label>
                    <div className="relative group">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="reg-email" className="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1">Електронна пошта</Label>
                    <div className="relative group">
                      <Mail className="absolute left-4 top-1/2 -translate-y-1/2 size-5 text-muted-foreground group-focus-within:text-primary transition-colors" />
                      <Input 
                        id="reg-email"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="reg-password" className="text-xs font-black uppercase tracking-widest text-muted-foreground ml-1">Пароль</Label>
                    <div className="relative group">
                      <Lock className="absolute left-4 top-1/2 -translate-y-1/2 size-5 text-muted-foreground group-focus-within:text-primary transition-colors" />
                      <Input 
                        id="reg-password"
              ...
              <div className="relative my-10">
                ...
                <div className="relative flex justify-center text-[10px] uppercase tracking-[0.3em] text-muted-foreground font-black">
                  <span className="bg-[#1a1226] px-4 rounded-full">АБО ЧЕРЕЗ</span>
                </div>
              </div>

              <Button variant="outline" className="w-full bg-white/5 border-white/10 hover:bg-white/10 h-16 rounded-[1.25rem] text-white font-bold flex items-center justify-center gap-3 transition-all" onClick={handleGoogleLogin}>
