...
export function AppSidebar() {
  ...
  const membershipsQuery = React.useMemo(() => (user ? query(
  ...
  const classRooms = React.useMemo(() => {
    const list = [...(userMemberships?.filter((m: any) => m.type === "classroom") || [])];
    if (list.length === 0 && profile?.gradeLevel && profile?.schoolId) {
      const normalizedClass = profile.gradeLevel.trim().replace(/\s+/g, '-').toUpperCase();
      const virtualId = `class-${profile.schoolId}-${normalizedClass}`;
      list.push({ id: virtualId, name: profile.gradeLevel.trim(), type: "classroom" });
    }
    return list;
    ...
    <Sidebar collapsible="icon" className="border-r border-white/5 bg-[#0d091a]">
      ...
      <SidebarContent className="px-2 py-2">
        <SidebarMenu>
            ...
            return (
              <SidebarMenuItem key={item.href}>
                <SidebarMenuButton asChild isActive={isActive} className={cn(
                  ...
                  <Link href={item.href} onClick={() => setOpenMobile(false)} className="flex items-center">
                    <div className={cn(
          ...
          <SidebarMenuItem>
            <SidebarMenuButton asChild isActive={pathname === '/support'} className="h-10 rounded-xl mb-1 hover:bg-white/5 group">
              <Link href="/support" onClick={() => setOpenMobile(false)} className="flex items-center">
                ...
                {!isCollapsed && <span className="font-bold text-xs ml-3 text-muted-foreground group-hover:text-white">Підтримка</span>}
              </Link>
            </SidebarMenuButton>
          </SidebarMenuItem>
          ...
          {isAdmin && (
            <SidebarMenuItem>
              <SidebarMenuButton asChild isActive={pathname === '/admin'} className="h-10 rounded-xl mb-1 hover:bg-destructive/10 group">
                <Link href="/admin" onClick={() => setOpenMobile(false)} className="flex items-center">
                  <div className="size-7 rounded-lg bg-destructive/10 flex items-center justify-center shrink-0">
                    <ShieldAlert className="size-4 text-destructive" />
                  </div>
                  {!isCollapsed && <span className="font-bold text-xs ml-3 text-destructive">Адмін</span>}
                </Link>
              </SidebarMenuButton>
            </SidebarMenuItem>
          )}
        ...
        {classRooms.length > 0 && (
          <div className="mt-6 px-2">
            {!isCollapsed && <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 mb-2 px-2">Кімнати класу</p>}
            <SidebarMenu>
              {classRooms.map((room: any) => {
                const isChatActive = pathname === `/chat/${room.id}`;
                return (
                  <SidebarMenuItem key={room.id}>
                    <SidebarMenuButton asChild isActive={isChatActive} className="h-9 rounded-lg mb-1 hover:bg-white/5">
                      <Link href={`/chat/${room.id}`} onClick={() => setOpenMobile(false)} className="flex items-center">
                        <div className="size-6 rounded-md bg-primary/10 flex items-center justify-center shrink-0">
                          <Hash className="size-3 text-primary" />
                        </div>
                        {!isCollapsed && <span className="font-bold ml-3 text-[11px] truncate">{room.name}</span>}
                      </Link>
                    </SidebarMenuButton>
                  </SidebarMenuItem>
                );
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <SidebarMenuButton className={cn(
              "h-auto p-3 rounded-2xl transition-all border border-white/5 bg-white/5 group relative",
              ...
              <Avatar className="size-9 border border-white/10 shrink-0">
                ...
                <AvatarFallback className="bg-primary/20 text-primary text-xs font-bold">{user?.displayName?.[0]}</AvatarFallback>
              </Avatar>
              {!isCollapsed && (
                <div className="flex flex-col items-start ml-3 overflow-hidden">
                  <span className={cn(
                    ...
                    {hasVIP && <Crown className="size-3 text-yellow-400 fill-yellow-400" />}
                  </span>
                  <span className={cn(
                    "text-[8px] font-black uppercase tracking-[0.15em] mt-1.5 opacity-80",
                    hasFire ? "text-white" : "text-primary"
                  )}>
                    {hasFire ? 'LEGEND ON FIRE' : (profile?.gradeLevel?.trim() || "Школяр")}
                  </span>
                </div>
              )}
            </SidebarMenuButton>
          ...
          <DropdownMenuContent side="right" align="end" className="w-52 glass-panel border-white/10 rounded-xl p-1 shadow-2xl">
            ...
            <DropdownMenuItem asChild className="rounded-lg h-9 cursor-pointer focus:bg-primary/20">
              <Link href="/settings" className="flex items-center w-full font-bold text-xs"><Settings2 className="size-4 mr-2 text-primary" /> Налаштування</Link>
            </DropdownMenuItem>
            <SidebarSeparator className="bg-white/5 my-1" />
            <DropdownMenuItem className="rounded-lg h-9 cursor-pointer text-destructive focus:bg-destructive/10 font-bold text-xs" onClick={handleLogout}>
